...
- **Meeting (บันทึกประชุม · MoM)** = หมุดบนแกนเวลา **ไม่ใช่งาน** — อยู่ข้าง ๆ ลำดับชั้นข้างบน ไม่ได้อยู่ในนั้น · สังกัดโปรเจกต์หนึ่ง หรือไม่สังกัดเลยก็ได้ (ประชุมทีม) · เนื้อในเป็น "บรรทัด" ที่ติดสัญลักษณ์: `[] ข้อความ` / `[x]` = สิ่งที่ต้องทำ · `#decision` = ข้อตกลง · `#question` `#parking` = เรื่องที่ยังไม่จบ · `#CO-142` = อ้างถึงงาน · `@ชื่อ` = คนที่ถูกพาดพิง · **สิ่งที่ต้องทำในประชุมกลายเป็น Task จริงได้ ทางเดียวเท่านั้น (ประชุม → งาน)** งานที่เกิดแบบนี้จะชี้กลับด้วย `meeting_id` · ตัวเลขที่สำคัญที่สุดของประชุมคือ **ของค้าง (`action_open_count`)** — เป็นเหตุผลเดียวที่คนกลับไปเปิดบันทึกเก่า
<!-- MODEL:END -->
