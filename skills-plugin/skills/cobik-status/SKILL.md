...
- **ข้อตกลงที่เคยคุยกันไว้** → `search_meeting_notes` (`tag: "decision"` · ใส่ `query` ได้ถ้ารู้คำ) — ตอบ "ตกลงกันว่ายังไง" โดยไม่ต้องรู้ว่าอยู่ประชุมครั้งไหน
- **ของค้างจากห้องประชุม** → `list_meetings` + `has_open_actions: true` (บวก `project_id` ถ้าเจาะโปรเจกต์เดียว) — สิ่งที่รับปากกันไว้แล้วยังไม่ได้ทำ มักไม่เคยขึ้นบอร์ด จึงไม่โผล่ใน `whats_overdue` เลย

## เขียน readout ยังไง (ไม่ใช่เท data)
สรุปเป็น 3 ส่วนเสมอ:
1. **บนเส้นทาง** — คืบหน้าเท่าไร อะไรเสร็จ
2. **ต้องระวัง** — overdue / unassigned / คนล้น (เกิน ~70–80%) / เดดไลน์ใกล้ / ของค้างจากประชุมที่ยังไม่ปิด
